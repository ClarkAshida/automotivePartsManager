...
class PartCarModelViewTests(APITestCase):
    ...
    # Teste de usuário admin conseguir editar associação
    def test_admin_can_edit_association(self):
        association = PartCarModel.objects.create(part=self.part1, car_model=self.car_model1)
        access_token = AccessToken.for_user(self.admin)
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {access_token}')
        ...
        data = {
            'part': self.part2.id,
            'car_model': self.car_model2.id
        }

        response = self.client.put(reverse('partcarmodel-detail', args=[association.id]), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
